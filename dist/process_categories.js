...
function dictRemovePlurals(dictionary) {
    const keysArray = Object.keys(dictionary);
    const keysToRemove = [];
    for (let i = 0; i < keysArray.length; i++) {
        // does this word end in "s"?
        let word = keysArray[i];
        let isS = keysArray[i].slice(-1) === "s" ? true : false;
        let isES = keysArray[i].slice(-2) === "es" ? true : false;
        let wordMinusS = keysArray[i].slice(0, -1);
        let wordMinusES = keysArray[i].slice(0, -2);
        if (isS) {
            // Does this word minus S exist as a key
            if (!!dictionary[wordMinusS]) {
                dictionary[wordMinusS] = dictionary[wordMinusS].concat(dictionary[word]);
                ...
                if (!!dictionary[wordMinusES]) {
                    dictionary[wordMinusES] = dictionary[wordMinusES].concat(dictionary[word]);
                    keysToRemove.push(word); //this key was merged so we remove it later from the main dictionary
                }
            }
        }
...
function logUniqueCategoriesArray(lastFunctionRun, x) {
    try {
